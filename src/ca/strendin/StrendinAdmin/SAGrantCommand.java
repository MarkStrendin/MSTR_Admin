...
public class SAGrantCommand implements CommandExecutor {
    ...
    public SAGrantCommand(StrendinAdmin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command arg1, String arg2,
        ...
        if (sender instanceof Player) {
            if (SAPermissions.canGrantLevels((Player)sender)) {
                    ...
                    if (arg3.length > 1) {
                        if (arg3[1].length() > 0) {                            
                            try {
                                ...
                                SAComms.sendError(player, "That doesn't seem to be a valid number");
                                return false;
                            }
                        }
                    }
                    ...
                    if (arg3[0].length() > 0) {                        
                        ...
                        if (target != null) {
                            if (target.getLevel() >= ignoreifplayerlevelabove) {
                                SAComms.sendError(player, target.getDisplayName() + " seems to have enough levels already!");
                                return true;
                            }
                            ...
                            SAComms.sendInfo(player, "Granting " + numlevels + " levels to " + target.getDisplayName());
                            target.setLevel(target.getLevel() + numlevels);
                        } else {
                            SAComms.sendError(player, "Player not found!");                                                    
                        }
                    }
                } else {
                    return false;
